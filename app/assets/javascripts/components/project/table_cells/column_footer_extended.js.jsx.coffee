...
ColumnFooterExtended = React.createClass
  ...
  propTypes:
    ...
    productVolumes   : React.PropTypes.object.isRequired
    contributoryData : React.PropTypes.object.isRequired
    comparisonRates  : React.PropTypes.any.isRequired
  ...
  render: ->
    ...
    total_cost = 0
    has_comparison_rates = !_.isEmpty(@props.comparisonRates)
    ...
    for index, product of @props.products
      ...
      unit_rate         = parseFloat(@props.unitRates[product_id])
      rate_denomination = parseInt(@props.rateDenominators[product_id])
      product_volume    = parseInt(@props.productVolumes[product_id])
      ...
      if @props.contributoryData[product_id] == true
        ...
        yearly_total = unit_rate * unit_basis * 12
      ...
      else if _.isBoolean(is_contributory)
        productRows.push(
          `<div className="wt-footercell__content" key={key}>
            <div className="wt-footercell__data-row">
    ...
    if @props.isPolicy
      ...
      for index, product of @props.products
        product_id       = product.product_id
        product_rate     = parseFloat(@props.comparisonRates[product_id])
        unit_basis       = (parseInt(@props.productVolumes[product_id]) / parseInt(@props.rateDenominators[product_id])) || null
        yearly_total     = product_rate * unit_basis * 12
        comparison_total = comparison_total + yearly_total if isUsableNumber(yearly_total)
