module ApplicationHelper
  ...
  def format_duration(ms)
    ...
    x = ms / 1000
    seconds = x % 60
    str.prepend("#{seconds.round} seconds")
    x /= 60
    minutes = x % 60
    str.prepend("#{minutes.round} minutes, ") if x.to_i > 0
    x /= 60
    hours = x % 24
    str.prepend("#{hours.round} hours, ") if x.to_i > 0
    str
  end

  # Gets ALL dynamic attributes that can be attached to the given object.  The results
  # are sorted according to the category and attribute orderings
  #
  # * *Args*    :
  #   - +obj+ -> object that has dynamic values
  # * *Returns* :
  #   - The attributes grouped by their category.  The return value is a hash of String -> Array
  def attributes_for_display(obj)
    attributes = DynamicAttribute.where(parent_class: obj.class.name)
                  .joins("LEFT JOIN categories ON dynamic_attributes.category_id = categories.id")
                  .includes(:product_types, :category)
                  .order("category_order ASC NULLS LAST, attribute_order ASC NULLS LAST").to_a

    if obj.respond_to?(:product_type)
      attributes.select! { |a| a.product_type_ids.empty? || a.product_type_ids.include?(obj.product_type.try(:id))}
    end

    attributes.group_by { |attr| attr.category.try(:name) || "General"}
  end
